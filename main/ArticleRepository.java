...
public class ArticleRepository {
	...
	public void addReply(int articleId, String body, String nickname) {

		Reply reply = new Reply(replyId, articleId, body, nickname, Util.getCurrentDate());
		replies.add(reply);
		replyId++;
	}

	public Reply getReplyOne(int id) {
		// id를 번호로 하는 게시물이 존재하는지, 존재하면 리턴.
		for (int i = 0; i < replies.size(); i++) {
			if (replies.get(i).getId() == id) {
	...
	public ArrayList<Reply> getRepliesByArticleId(int articleId) {
		ArrayList<Reply> searchedReplyList = new ArrayList<>();

		for (int i = 0; i < replies.size(); i++) {
			if (replies.get(i).getArticleId() == articleId) {
				searchedReplyList.add(replies.get(i));
			}
		}

		return searchedReplyList;
	}
